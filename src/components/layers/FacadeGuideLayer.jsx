...
const RISK_BG = { '#1ea64a': '#f0fdf4', '#f3c01b': '#fefce8', '#e4572e': '#fff1ee' };
const RISK_BORDER = { '#1ea64a': '#bbf7d0', '#f3c01b': '#fde68a', '#e4572e': '#fecaca' };

// Push a lat/lng point dist metres outward along a bearing.
...
export function FacadeGuideLayer({ building, wind }) {
  if (!building) return null;
  const windDir = wind?.direction ?? 0;
  const faces = buildFacadeGuide(building, windDir);
      ...
      <Marker key={f.cardinal} longitude={lng} latitude={lat} anchor="center">
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 3 }}>
            ...
            <span style={{ color: VENT_COLOR[f.vent.label.includes('inlet') ? 'inlet' : f.vent.label.includes('outlet') ? 'outlet' : 'neutral'], fontSize: 7, fontWeight: 600 }}>{f.vent.label}</span>
          </div>
        </div>
      </Marker>
    );
